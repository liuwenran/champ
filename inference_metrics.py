...
def combine_guidance_data(cfg, guidance_data_folder):
    ...
    for guidance_type in guidance_types:
        ...
        depth_dir = os.path.join(guidance_data_folder, guidance_type)
        IMAGE_EXTENSIONS = ('.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG')
        frame_files = os.listdir(depth_dir)
        frame_files = [os.path.join(depth_dir, f) for f in frame_files]
        frame_files.sort()
        all_guide_images = []
        for frame in frame_files:
            frame_extension = os.path.splitext(frame)[1]
            if frame_extension in IMAGE_EXTENSIONS:
                all_guide_images.append(frame)
        
        if len(all_guide_images) > 72:
            all_guide_images = all_guide_images[::3]

        all_guide_images = all_guide_images[:24]

        for guidance_image_path in all_guide_images:
        ...
        # ):
            ...
            if guidance_type == "semantic_map":
                guidance_pil_group[guidance_type] += [
                    process_semantic_map(Path(guidance_image_path))
                ]
            else:
                guidance_pil_group[guidance_type] += [
...
def main(cfg):
    ...
    video_names = [ path for path in os.listdir(cfg.data.dataset_root_dir)]
    for ind, video_name in enumerate(video_names):
        ref_random_idx = 0
        ref_image_path = os.path.join(cfg.data.dataset_root_dir, video_name, "images", '0001.png')
        guidance_folder_path = os.path.join(cfg.data.dataset_root_dir, video_name)

        # ref_image_path = cfg.data.ref_image_path
        ref_image_pil = Image.open(ref_image_path)
        ref_image_w, ref_image_h = ref_image_pil.size

        guidance_pil_group, video_length = combine_guidance_data(cfg, guidance_folder_path)
        ...
        save_videos_grid(result_video_tensor, osp.join(save_dir, f"video_{cfg.exp_name}_{video_name}_{ref_random_idx}.mp4"))
        ...
        # save_videos_grid(grid_video, osp.join(save_dir, f"{dataset_name}_{video_name}_grid.mp4"))
        # save_videos_grid(grid_video_wguidance, osp.join(save_dir, f"{dataset_name}_{video_name}_grid_wguidance.mp4"))
